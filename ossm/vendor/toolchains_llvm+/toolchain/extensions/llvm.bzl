...
def _llvm_impl_(module_ctx):
    ...
    for mod in module_ctx.modules:
        ...
        for root in mod.tags.target_toolchain_root:
            if root.name not in toolchain_names_for_module:
                fail("target_toolchain_root '%s' does not have a corresponding toolchain in module '%s'" % (root.name, mod.name))
        ...
        for tag in mod.tags.extra_compiler_files:
            if tag.name not in toolchain_names_for_module:
                fail("extra_compiler_files '%s' does not have a corresponding toolchain in module '%s'" % (tag.name, mod.name))
        for tag in mod.tags.extra_linker_files:
            if tag.name not in toolchain_names_for_module:
                fail("extra_linker_files '%s' does not have a corresponding toolchain in module '%s'" % (tag.name, mod.name))
