...
*F*/
/* clang-format off */
IPPFUN(IppStatus, ippsSMS4EncryptECB,(const Ipp8u* pSrc,
                                      Ipp8u* pDst,
                                      int len,
...
/* do encryption */
    ...
    if (IsFeatureEnabled(ippCPUID_AVX2SM4)) {
        for (; len >= 2 * MBS_SMS4;
             len -= 2 * MBS_SMS4, pSrc += 2 * MBS_SMS4, pDst += 2 * MBS_SMS4) {
            cpSMS4_ECB_ni_256(pDst, pSrc, SMS4_RK(pCtx));
        }
        for (; len > 0; len -= MBS_SMS4, pSrc += MBS_SMS4, pDst += MBS_SMS4) {
            cpSMS4_ECB_ni(pDst, pSrc, SMS4_RK(pCtx));
        }
    } else
#endif
