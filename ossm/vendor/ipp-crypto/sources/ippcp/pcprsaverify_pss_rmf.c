...
*F*/
/* clang-format off */
IPPFUN(IppStatus, ippsRSAVerify_PSS_rmf, (const Ipp8u* pMsg,
                                          int msgLen,
                                          const Ipp8u* pSign,
                                          int* pIsValid,
...
/* clang-format on */
{
    // badargs and pointer alignments, set *pIsValid = 0
    const IppStatus preprocResult =
        SingleVerifyPssRmfPreproc(pMsg, msgLen, pSign, pIsValid, &pKey, pMethod, pScratchBuffer);
