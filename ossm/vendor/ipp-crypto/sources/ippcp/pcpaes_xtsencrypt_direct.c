...
//
*F*/
static IppStatus cpAES_XTS_EncBlock(Ipp8u* ctxt,
                                    const Ipp8u* ptxt,
                                    const Ipp8u* tweak,
...
/* clang-format off */
IPPFUN(IppStatus, ippsAESEncryptXTS_Direct, (const Ipp8u* pSrc,
                                             Ipp8u* pDst,
                                             int encBitsize,
                                             int aesBlkNo,
         ...
         */
...
#endif
            {
                for (; encBlocks >= AES_BLKS_PER_BUFFER && ippStsNoErr == sts;
                    ...
                    sts = ippsAESEncryptECB(tmpDst,
                                            pDst,
                                            AES_BLKS_PER_BUFFER * AES_BLK_SIZE,
                                            &aesCtx);
