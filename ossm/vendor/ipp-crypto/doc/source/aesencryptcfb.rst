...
IppStatus ippsAESEncryptCFB(const Ipp8u\* pSrc, Ipp8u\* pDst, int
srcLen, int cfbBlkSize, const IppsAESSpec\* pCtx, const Ipp8u \*pIV);
...
The function encrypts the input data stream of variable length according
to the CFB mode as specified in :term:`NIST SP 800-38A <[NIST SP 800-38A]>`.
...
.. list-table::
   ...
   * - ippStsUnderRunErr
     - Indicates an error condition if srcLenis not divisible by cfbBlkSize parameter value.
   * - ippStsCFBSizeErr
